...
#
#login
if(isset($_POST['uid']) and isset($_POST['year']) and isset($_POST['month']) and isset($_POST['day']) and isset($_POST['us_ty'])){
	
	if(!is_numeric(trim($_POST['uid']))){
		die('Admission Number has to be Numeric');
	}
	
	
	if(!is_numeric(trim($_POST['year']))){
		die('Year has to be Numeric');
	}
	
	if(!is_numeric(trim($_POST['month']))){
		die('Month has to be Numeric');
	}
	
	if(!is_numeric(trim($_POST['day']))){
		die('Day has to be Numeric');
	}
	if(!is_numeric(trim($_POST['us_ty']))){
		die('Day has to be Numeric');
		if($_POST['us_ty'] == 1 or $_POST['us_ty'] == 2 ){
		}else{
			die('Invalid Type Values');
		}
	}
	...
	if(strlen($_POST['month']) === 1){
		...
		$eml=$_POST['uid'];
	$pas=md5(md5(sha1($_POST['year'].$_POST['month'].$_POST['day'])));
	$pass_raw = ($_POST['year'].$_POST['month'].$_POST['day']);
	$pass_raw_o = ($_POST['year'].'-'.$_POST['month'].'-'.$_POST['day']);
...
lum_type = 1 and
lum_username= '".$eml."' and
lum_password = '".$pas."' and
lum_hash= '".$hash."' and
lum_y ='".$_POST['year']."' and
lum_m ='".$_POST['month']."' and
lum_d ='".$_POST['day']."' and 
lum_valid = 1
...
if ($usrres->num_rows == 1) {
    ...
    while($usrrw = $usrres->fetch_assoc()) {
		...
		$_SESSION['SBSEVOTE_SESS_ID'] = md5(sha1(md5(md5(sha1('SecretBall')).uniqid().time()).time()).uniqid());
...
if($_POST['us_ty'] == 1){
	...
	curl_setopt_array($curl, array(
		...
		CURLOPT_URL => 'http://stepsys.org/auth/auth.cfm?uid='.$eml.'&pw='.$pass_raw,
		CURLOPT_USERAGENT => 'cURL Request AYAN'
	));
	// Send the request & save response to $resp
	$resp = curl_exec($curl);
	// Close request to clear up some resources
	curl_close($curl);
}elseif($_POST['us_ty'] ==2){
	$url = "http://www.stepsys.org/auth/teachers.cfm";
	...
	curl_setopt_array($curl, array(
		CURLOPT_RETURNTRANSFER => 1,
		CURLOPT_URL => 'http://stepsys.org/auth/emp_auth.cfm?uid='.$eml.'&pw='.$pass_raw,
		CURLOPT_USERAGENT => 'cURL Request AYAN'
	));
	// Send the request & save response to $resp
	$resp = curl_exec($curl);
...
if(!is_numeric(trim($house))){
	die('Invalid House');
}

if($_POST['us_ty'] == 2){
	if(is_numeric(trim($house)) and (trim($house) == 0)){
	}else{
		die("Invalid House Staff");
	}	
}else{
	if((trim($house) == 1) or (trim($house) == 2) or (trim($house) == 3) or (trim($house) == 4)){
	}else{
		die("Invalid House");
	}
}

$minclassforvoting = getdatafromsql($conn,"SELECT min(pos_class_valid) as minclass FROM c_master_positions where pos_valid =1 and pos_user_type=".$_POST['us_ty']);

if(is_array($minclassforvoting)){
}else{
	die('ErrMAFINDINGMINCLASS');
}
if(trim($class) >= $minclassforvoting['minclass']){
}else{
	header('Location: login.php?errflag_li_notelig');
	die();
}
...
$sqla = "
INSERT INTO `b_sm_logins`(`lum_image`, `lum_type`,`lum_username`, `lum_password`, `lum_hash`,`lum_valid`,`lum_name`,`lum_y`, `lum_m` ,`lum_d` ,`lum_class` ,`lum_section` ,`lum_house`) VALUES (
'".trim($imagename)."',
'".trim($_POST['us_ty'])."',
'".trim($eml)."',
'".trim($pas)."',
...
if ($conn->query($sqla) === TRUE) {
	...
	$newuser = getdatafromsql($conn,"SELECT * FROM b_sm_logins where 
lum_id = '".$new_usr_id."' and lum_valid = 1
");
...
if(is_array($newuser)){
	...
	session_regenerate_id();
		...
		$_SESSION['SBSEVOTE_SESS_ID'] = md5(sha1(md5(md5(sha1('SecretBall')).uniqid().time()).time()).uniqid());
		$_SESSION['EVT_USR_DB_ID'] = trim($new_usr_id);
	session_write_close();
...
header('Location: home.php');
	...
	die('ERRMAOIRFJO8EJRF9U3HERUGHU');
}
...
*/
	...
	$sqllli = "INSERT INTO b_sv_auth_fail ( `af_adm_no`, `af_dob`, `af_sess_hash`, `af_ip`, `af_dnt`, `af_server_dump`)
							VALUES ('".$eml."', '".$pass_raw_o."', '".$_SESSION['SESS_USR_LOG_MS_VIEW_MD5_ID']."',
